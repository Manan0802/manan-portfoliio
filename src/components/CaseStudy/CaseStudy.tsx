...
const architectureData: Record<string, { nodes: Node[]; edges: Edge[] }> = {
  neofin: {
    nodes: [
      { id: '1', position: { x: 50, y: 150 }, data: { label: '📱 PWA Client (React)' }, type: 'input' },
      { id: '2', position: { x: 250, y: 150 }, data: { label: '🌐 Cloudflare CDN/WAF' } },
      { id: '3', position: { x: 450, y: 150 }, data: { label: '⚡ Node.js API Gateway' } },
      { id: '4', position: { x: 450, y: 50 }, data: { label: '🔐 JWT Auth Service' } },
      { id: '5', position: { x: 650, y: 150 }, data: { label: '📨 BullMQ Async Queue' } },
      { id: '6', position: { x: 850, y: 50 }, data: { label: '🤖 Gemini 2.5 Flash' } },
      { id: '7', position: { x: 850, y: 150 }, data: { label: '⚙️ Python Worker' } },
      { id: '8', position: { x: 1050, y: 250 }, data: { label: '🗄️ MongoDB Atlas' }, type: 'output' },
      { id: '9', position: { x: 650, y: 250 }, data: { label: '🚀 Redis Cache' } },
    ],
    edges: [
      ...
      { id: 'e9-8', source: '9', target: '8' },
    ],
  },
  shoplens: {
    nodes: [
      { id: '1', position: { x: 50, y: 100 }, data: { label: '🔎 Streamlit Client' }, type: 'input' },
      { id: '2', position: { x: 250, y: 100 }, data: { label: '🛡️ FastAPI Edge' } },
      { id: '3', position: { x: 250, y: 0 }, data: { label: '🪣 AWS S3 (Raw Images)' } },
      { id: '4', position: { x: 450, y: 100 }, data: { label: '🧠 CLIP Embedding Model' } },
      { id: '5', position: { x: 650, y: 0 }, data: { label: '📉 Dim Reduction (PCA)' } },
      { id: '6', position: { x: 650, y: 200 }, data: { label: '⚡ FAISS Vector Index' } },
      { id: '7', position: { x: 850, y: 100 }, data: { label: '🛒 Similarity Ranker' } },
      { id: '8', position: { x: 1050, y: 100 }, data: { label: '📊 JSON Results' }, type: 'output' },
    ],
    edges: [
      { id: 'e1-2', source: '1', target: '2', animated: true },
      { id: 'e2-3', source: '2', target: '3' },
  ...
  investmate: {
    nodes: [
      { id: '1', position: { x: 50, y: 150 }, data: { label: '📈 Trading Dashboard' }, type: 'input' },
      { id: '2', position: { x: 250, y: 150 }, data: { label: '🔌 WebSockets Server' } },
      { id: '3', position: { x: 450, y: 50 }, data: { label: '🕒 Celery Chron Jobs' } },
      { id: '4', position: { x: 450, y: 150 }, data: { label: '📡 FastAPI Backend' } },
      { id: '5', position: { x: 650, y: 50 }, data: { label: '📊 AlphaVantage API' } },
      { id: '6', position: { x: 650, y: 250 }, data: { label: '🤖 Gemini Risk Analyzer' } },
      { id: '7', position: { x: 850, y: 150 }, data: { label: '🗄️ PostgreSQL (Timescale)' }, type: 'output' },
      { id: '8', position: { x: 250, y: 250 }, data: { label: '🚀 Redis Pub/Sub' } },
    ],
    edges: [
      { id: 'e1-2', source: '1', target: '2', animated: true },
  ...
  'crop-yield': {
    nodes: [
      { id: '1', position: { x: 50, y: 100 }, data: { label: '🚜 Farmer Next.js App' }, type: 'input' },
      { id: '2', position: { x: 250, y: 100 }, data: { label: '🌩️ API Gateway' } },
      { id: '3', position: { x: 450, y: 0 }, data: { label: '🌧️ Weather API' } },
      { id: '4', position: { x: 450, y: 200 }, data: { label: '🌱 Soil Sensor DB' } },
      { id: '5', position: { x: 650, y: 100 }, data: { label: '🐋 Docker Inference Container' } },
      { id: '6', position: { x: 850, y: 100 }, data: { label: '🧠 PyTorch Estimator' } },
      { id: '7', position: { x: 1050, y: 100 }, data: { label: '📋 Actionable Report' }, type: 'output' },
    ],
    edges: [
      { id: 'e1-2', source: '1', target: '2', animated: true },
...
const defaultArchitecture = (title: string): { nodes: Node[]; edges: Edge[] } => ({
  nodes: [
    { id: '1', position: { x: 50, y: 100 }, data: { label: `🖥️ ${title} Web Client` }, type: 'input' },
    { id: '2', position: { x: 250, y: 100 }, data: { label: '🌐 Load Balancer' } },
    { id: '3', position: { x: 450, y: 0 }, data: { label: '🔐 Auth Service' } },
    { id: '4', position: { x: 450, y: 200 }, data: { label: '🚀 Redis Cache' } },
    { id: '5', position: { x: 650, y: 100 }, data: { label: '⚙️ Microservices K8s' } },
    { id: '6', position: { x: 850, y: 100 }, data: { label: '🗄️ Primary RDS / NoSQL' }, type: 'output' },
  ],
  edges: [
    { id: 'e1-2', source: '1', target: '2', animated: true },
    { id: 'e2-5', source: '2', target: '5' },
